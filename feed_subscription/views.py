...
from feed_subscription.selectors import subscription_exists, get_user_existing_subscriptions
from feed_subscription.serializers import FeedSubscriptionReadOnlySerializer
from feed_subscription.services import subscribe_to_feed, delete_subscription


class FeedSubscriptionAPI(APIView):
    ...
    def post(self, request: Request):
        ...
        sr.is_valid(True)
        sub = subscribe_to_feed(user=request.user, **sr.validated_data)
        sr = FeedSubscriptionReadOnlySerializer(sub, request=request)
        return ok(sr.data)

    def get(self, request):
        paginator = BasePageNumberPagination()
        query_set = get_user_existing_subscriptions(user=request.user)
...
class FeedSubscriptionRetrieveAPI(APIView):
    def delete(self, request, subscription_id):
        sub = delete_subscription(user=request.user, subscription_id=subscription_id)
        sr = FeedSubscriptionReadOnlySerializer(sub, request=request)
        return ok(sr.data)
